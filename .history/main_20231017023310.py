...
class WindowTracker:
    def __init__(self):
        self.df = pd.DataFrame(columns=['Timestamp', 'Application', 'Window_Title', 'Time_Spent', 'Key_Strokes', 'Mouse_Clicks', 'Session_Duration', 'Active', 'Frequency'])
        self.load_config()
    ...
    def capture_window_title(self):
        ...
        if window != None:
            ...
            session_duration = self.calculate_session_duration(title, current_time)
            return current_time, app, title, time_spent, self.key_count, self.mouse_clicks, session_duration, active
        return None, None, None, None, None, None, None, None

    def calculate_session_duration(self, title, current_time):
        if title != self.prev_title:
            self.window_changes += 1
        self.prev_title = title
        return current_time - self.prev_time  # This needs to be adapted to your needs
    ...
    def update_data(self, timestamp, app, title, time_spent, key_strokes, mouse_clicks, session_duration, active):
        new_row = pd.DataFrame([[timestamp, app, title, time_spent, key_strokes, mouse_clicks, session_duration, active, self.window_changes]], columns=self.df.columns)
...
def run(self):
    ...
    try:
        while True:
            timestamp, app, title, time_spent, key_strokes, mouse_clicks, session_duration, active = self.capture_window_title()
            if timestamp:
                self.update_data(timestamp, app, title, time_spent, key_strokes, mouse_clicks, session_duration, active)
